...
import 'package:flutter_pengaduan_application/models/masyarakat.dart';
...
class MasyarakatController extends GetxController {
  ...
  getData() async {
    try {
      ...
      loading.value = true;
      var response =
          await fetch.get(Uri.parse("http://localhost:3320/masyarakat"));
      loading.value = false;
  ...
  createData(String nik, String nama, String username, String password,
    ...
    try {
      ...
      var response = await fetch.post(
        Uri.parse("http://localhost:3320/masyarakat"),
        body: jsonEncode({
          "nik": nik,
          "nama": nama,
          "username": username,
  ...
  updateData(id, String nama, String username, String telp) async {
    try {
      loading.value = true;
      var response = await fetch.patch(
        Uri.parse("http://localhost:3320/masyarakat/${id}"),
        body: jsonEncode({
          "nama": nama,
          "username": username,
          "telp": telp,
